...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="div" mode="index">
        <xsl:value-of select="$div_index"/> <!--richiamo var-->
        <ul>
            <xsl:for-each select=".">
                <li>
    ...
    <xsl:param name="style_title">font-weight:bold</xsl:param> <!--parametri-->
        <xsl:call-template name="title"> <!--call-template-->
            <xsl:with-param name="style_title">
                <h2>
                    <xsl:value-of select="." />
                </h2>
            </xsl:with-param>
    ...
    <xsl:template match="tei:persName">
        <xsl:param name="style" >text-decoration:underline</xsl:param> 
        <a href="http://">
